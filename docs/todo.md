...
- [ ] Redirect resolution for `vertexaisearch` sources URLs now happens on every Sources button retrieval. Add an LRU cache for it.
...
## Bugfixes
- [ ] Triage model sometimes calls video / URL fetch content tool on URLs with content already in the history chain. These calls can (and should be) be filtered programatically instead of doubling data unnecessarily in the conversation history.