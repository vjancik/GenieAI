...
import type { IDiscordMessagePageRepository, DiscordMessagePage } from '../../core/domain/repositories/discord-message-page-repository';
...
export class PostgresDiscordMessagePageRepository implements IDiscordMessagePageRepository {
    ...
    async create(page: Omit<DiscordMessagePage, 'id'>): Promise<string> {
        const [inserted] = await this.db.insert(discordMessagePages).values({
    ...
    async findById(id: string): Promise<DiscordMessagePage | null> {
        const [page] = await this.db.select()
            .from(discordMessagePages)
            .where(eq(discordMessagePages.id, id));
    ...
    async delete(id: string): Promise<void> {
        await this.db.delete(discordMessagePages)
            .where(eq(discordMessagePages.id, id));
    }
}
