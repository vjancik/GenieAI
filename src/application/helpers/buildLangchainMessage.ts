 ...
 */
function buildTokenContentParts(
    content: string,
 ...
 */
export function buildLangchainMessage<R extends "human" | "assistant">(params: {
    role: R;
    content: string;
    attachments: IChatClientMessageAttachment[];
    embeds?: IChatClientMessageEmbed[];
    ...
    /** Encoded into discord:// token URLs. Required when attachments or embeds are present. */
    discordMessageId?: string;
}): R extends "human" ? HumanMessage : AIMessage {
    ...
    return wrap(buildTokenContentParts(content, attachments, embeds, guildId, channelId, discordMessageId));
}
