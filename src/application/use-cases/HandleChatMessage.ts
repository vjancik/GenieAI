...
import { type AppConfig, AttachmentMode, SearchMode } from "../config/AppConfig.ts";
...
export class HandleChatMessageUseCase {
    ...
    private async persistPendingGeminiRecords(pendingRecords: PendingGeminiRecord[], messageId: string): Promise<void> {
        ...
        if (!this.geminiFileRepo || !this.geminiFileUploader) {
            throw new Error("Upload mode repository dependencies not injected into HandleChatMessage");
        }
        ...
        return Sentry.startSpan(
            ...
            async () => {
                if (!this.diskDownloader || !this.geminiFileUploader) {
                    throw new Error("Upload mode dependencies not injected into HandleChatMessage");
                }
